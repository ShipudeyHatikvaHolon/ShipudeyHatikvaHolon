...
switch($action) {
		case 'delete':
			...
			if(!is_numeric($id)) {
				header('location: pitasAdmin.php');
				exit;
			}

			$sql = "DELETE FROM pitas WHERE id = " . $id;
		...
		if(!is_numeric($id)) {
			...
			$sql = "SELECT * FROM pitas WHERE id = " . $id;
			$result = mysqli_query($conn, $sql);
		...
		case 'saveEdit':
			...
			if(!$name) {
				header('Location: pitasAdmin.php?errorMessage=שם המוצר הוא שדה חובה');
				exit;
			}

			if(!$min_amount) {
				header('Location: pitasAdmin.php?errorMessage=כמות מינימלית של המוצר הוא שדה חובה');
				exit;
			}

			if(!$amount) {
				header('Location: pitasAdmin.php?errorMessage=כמות המוצר הוא שדה חובה');
				exit;
			}

			$name = mysqli_real_escape_string($conn, $name);

			$sql = "UPDATE pitas SET
								`name` = '$name',
								`amount` = '$amount',
			...
			if( $result ) {
				header('Location: pitasAdmin.php?message=המוצר עודכן בהצלחה');
			}else {
				header('Location: pitasAdmin.php?errorMessage=קרתה בעיה בעדכון המוצר, אנא נסה שוב מאוחר יותר');
			}
			exit;
		...
		case 'saveNew':
			...
			$sql = "INSERT INTO pitas (id, `name`, amount, min_amount, price )
													VALUES('', '$name', '$amount', '$min_amount', '$price' )";

			$result = mysqli_query($conn, $sql);
						
			if( $result ) {
				header('Location: pitasAdmin.php?message=המוצר נשמר בהצלחה');
			}else {
				header('Location: pitasAdmin.php?errorMessage=קרתה בעיה בשמירת המוצר, אנא נסה שוב מאוחר יותר');
			}
			exit;
...
<body>
	...
	<div class="container">
			...
			<div class="tabs">
					...
					<div class="current tab"><a href="pitasAdmin.php">ניהול ארוחות</a></div>
		...
		<div style="display: flex; justify-content: center;">
			<a class="btn btn-info" href="pitasAdmin.php?action=addNew"> הוסף ארוחה חדשה </a>
		</div>
	...
	<script>
		$('.edit-button').on('click', function() {
			var id = $(this).data('id');
			window.location.href = "pitasAdmin.php?action=edit&id=" + id;
		})
		...
		$('.delete-button').on('click', function(e) {
			...
			if(r) {
				window.location.href = "pitasAdmin.php?action=delete&id=" + id;
			}
		...
		<?php 
			$title = is_numeric($id) ? 'עריכת ארוחה' : 'הוספת ארוחה לחנות';
			$actionLink = is_numeric($id) ? 'saveEdit' : 'saveNew';
			$submit_name = is_numeric($id) ? 'שמור' : 'הוסף ארוחה לחנות';
...
.button--plain {
	...
	<div class="container">
		...
		<p>
			...
			מחיר : <input type="text" name="price" value="<?php echo $id ? $product['price'] : '' ?>"><br>
