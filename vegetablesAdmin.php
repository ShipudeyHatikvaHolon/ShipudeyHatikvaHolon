...
switch($action) {
		case 'delete':
			...
			if(!is_numeric($id)) {
				header('location: vegetablesAdmin.php');
				exit;
			}
		...
		case 'saveEdit':
			...
			if(!$name) {
				header('Location: vegetablesAdmin.php?errorMessage=שם המוצר הוא שדה חובה');
				exit;
			}

			if(!$min_amount) {
				header('Location: vegetablesAdmin.php?errorMessage=כמות מינימלית של המוצר הוא שדה חובה');
				exit;
			}

			if(!$amount) {
				header('Location: vegetablesAdmin.php?errorMessage=כמות המוצר הוא שדה חובה');
				exit;
			}
			...
			if( $result ) {
				header('Location: vegetablesAdmin.php?message=המוצר עודכן בהצלחה');
			}else {
				header('Location: vegetablesAdmin.php?errorMessage=קרתה בעיה בעדכון המוצר, אנא נסה שוב מאוחר יותר');
			}
			exit;
		...
		case 'saveNew':
			...
			$sql = "INSERT INTO adds (`id`, `name`, `amount`, `min_amount` )
													VALUES('', '$name', '$amount', '$min_amount' )";

			$result = mysqli_query($conn, $sql);
						
			if( $result ) {
				header('Location: vegetablesAdmin.php?message=המוצר נשמר בהצלחה');
			}else {
				header('Location: vegetablesAdmin.php?errorMessage=קרתה בעיה בשמירת המוצר, אנא נסה שוב מאוחר יותר');
			}
			exit;
...
<body>
	...
	<div class="container">
			...
			<div class="tabs">
					<div class="current tab"><a href="vegetablesAdmin.php">ניהול תוספות</a></div>
		...
		<div style="display: flex; justify-content: center;">
			<a class="btn btn-info" href="vegetablesAdmin.php?action=addNew"> הוסף תוסף חדש </a>
		</div>
	...
	<script>
		$('.edit-button').on('click', function() {
			var id = $(this).data('id');
			window.location.href = "vegetablesAdmin.php?action=edit&id=" + id;
		})
		...
		$('.delete-button').on('click', function(e) {
			...
			if(r) {
				window.location.href = "vegetablesAdmin.php?action=delete&id=" + id;
			}
		...
		<?php 
			$title = is_numeric($id) ? 'עריכת תוסף' : 'הוספת מוצר לחנות המוצרים';
			$actionLink = is_numeric($id) ? 'saveEdit' : 'saveNew';
			$submit_name = is_numeric($id) ? 'שמור' : 'הוסף תוסף לחנות';
